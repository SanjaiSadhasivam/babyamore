...
const Login = () => {
  ...
  const Getotp = async (e) => {
    ...
    try {
      const result = await axios.post(API_Login_Otp, formData, config);
      if (result.data.statusCode == 200) {
        navigate("/otplogin", { state: Login.email_address });
      }
    } catch (error) {
  ...
  const Getotpemail = async (e) => {
    e.preventDefault();
    let formData = new FormData();
    formData.append("email_address", Login.email_address);
    formData.append("emailotp", true);
    try {
      const result = await axios.post(API_Login_Otp, formData, config);
      if (result.data.statusCode == 200) {
        navigate("/otplogin", { state: Login.email_address });
                  ...
                  <form onSubmit={Submit}>
                    <div className="login-form">
                      <div className="company_name focus-visble input">
                        <p> {isTrue ? "Mobile Number" : "Email Address"}</p>
                        <i className="fa-solid icon">
                          {isTrue ? <FaMobileAlt /> : <FaEnvelope />}
                        </i>
                        <div className="d-flex align-items-center">
                          <input
                            type={isTrue ? "telNo" : "email"}
                            className="sty2 mb-0"
                            name="email_address"
                            value={Login.email_address}
                            onChange={handleChangelogin}
                            minLength={isTrue ? "10" : "4"}
                            maxLength={isTrue ? "15" : "40"}
                            placeholder={
                              isTrue
                                ? "Enter Your Mobile Number"
                                : "Enter Your Email"
                            }
                            onKeyPress={isTrue?(event) => {
