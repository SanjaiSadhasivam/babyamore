...
const StaffManagement = ({ alter, id }) => {
  ...
  //   try {
  //     const Result = await axios.get(`${API_WareHouse}/?vendor_id=${id}`, config);
  ...
  const Getdata = async (id) => {
    const data = await axios.get(`${API_Staff}/?vendor_id=${vendarId}`, { headers: { Authorization: `Bearer ${token} ` } });
        ...
        <Block className="container ">
          <Row>
            ...
            <Col md={12}>
              ...
              <CustomDataTable icons={tableIcons} data={data} columns={staffmanagementTable} title="STAFF LIST" />
            </Col>
          </Row>
          <hr></hr>
          ...
          <Modal isOpen={view.Viewdetails} toggle={() => onFormCancel()} className="modal-dialog-centered" size="md">
            <ModalBody>
              <a className="close">
                {" "}
                <Icon
