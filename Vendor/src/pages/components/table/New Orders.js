...
const NewOrders = () => {
    ...
    const Getdata = async () => {
        ...
        const getd = neworders.data.list.filter((currEle) => currEle.vendor_id == cookies.vendor_id);
        console.log("new orders",getd)
        setNewOrderData(getd);
    }
    ...
    const GetFilterSearch = async (e) => {
        ...
        if (fromdate || todate || shiptype || shipcompany || sku || customers) {
            let NeworderStatus ='New orders';
            let Filt = API_Order + '/?' + (fromdate ? (`From_date=${fromdate}`) : '') + (todate ? (`&To_date=${todate}`) : '') + (shiptype ? (`&shippingtype=${shiptype.value}`) : '') + (shipcompany ? (`&shipcompany=${shipcompany.value}`) : '') + (sku ? (`&sku=${sku}`) : '') + (customers ? (`&customer=${customers}`) : '') + (NeworderStatus ? (`&order_status=${NeworderStatus}`) : '')
            // console.log("Filt", Filt);
            const Resultt = await axios.get(`${Filt}`, config);
            // console.log("resulttt", Resultt);
            // setNewOrderData(Resultt.data.list);

            const getdd = Resultt.data.list.filter((currEle) => currEle.vendor_id == cookies.vendor_id);
            // console.log("all orders", getd)
            setNewOrderData(getdd);
