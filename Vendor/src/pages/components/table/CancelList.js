...
import { API_URL, token, API_Cancel, API_Order, API_Product } from "../../../Api";
...
const Cancelled = () => {
  ...
  const Getdata = async () => {
    const Result = await axios.get(`${API_Cancel}`, config);
    const vendordata = Result.data.list.filter((currEle) => currEle.vendor_id == cookies.vendor_id);
