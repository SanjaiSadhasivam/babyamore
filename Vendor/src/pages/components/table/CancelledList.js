...
const Cancelled = () => {
    ...
    const Getdata = async () => {
        ...
        let Can = "Cancelled"
        let Intransit = await axios.get(`${API_Order}/?order_status=${Can}`, config);
        const vendordata = Intransit.data.list.filter((currEle) => currEle.vendor_id == cookies.vendor_id);
        // console.log("Intransit", Intransit)
        setIntransitData(vendordata);
    }
