...
const PendingHandover = () => {
    ...
    const [PendingHdoData, setPendingHdoData] = useState([]);
    ...
    const Getdata = async () => {
        let PendHand = "Ready to Handover"
        let Pendinghand = await axios.get(`${API_Order}/?order_status=${PendHand}`, config);
        // setPendingHdoData(Pendinghand.data.list);

        const getd = Pendinghand.data.list.filter((currEle) => currEle.vendor_id == cookies.vendor_id);
        console.log("pending handover",getd)
        setPendingHdoData(getd);
    }
    ...
    const GetFilterSearch = async (e) => {
        ...
        if (fromdate || todate || shiptype || shipcompany || sku || customers) {
            let PendingHandoverStatus = 'Pending Handover';

            let Filt = API_Order + '/?' + (fromdate ? (`From_date=${fromdate}`) : '') + (todate ? (`&To_date=${todate}`) : '') + (shiptype ? (`&shippingtype=${shiptype.value}`) : '') + (shipcompany ? (`&shipcompany=${shipcompany.value}`) : '') + (sku ? (`&sku=${sku}`) : '') + (customers ? (`&customer=${customers}`) : '') + (PendingHandoverStatus ? (`&order_status=${PendingHandoverStatus}`) : '')
            // console.log("Filt", Filt);
            const Resultt = await axios.get(`${Filt}`, config);
            // console.log("resulttt", Resultt);
            // setPendingHdoData(Resultt.data.list)
            
            const getdd = Resultt.data.list.filter((currEle) => currEle.vendor_id == cookies.vendor_id);
            // console.log("all orders", getd)
            setPendingHdoData(getdd);;
                    ...
                    <li className="mr-n1">
                        ...
                        <DropdownItem tag="a" href="#quickview">
                          ...
                          <span>Quick View</span>
                                    ...
                                    <li>
                                        <DropdownItem tag="a" onClick={() => View(row.id)}>
                                            <Icon name="eye"></Icon>
                                            <span>Full View</span>
                                        </DropdownItem>
                                    </li>
